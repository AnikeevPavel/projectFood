...
function cards() {
  class Menu {
    ...
    render() {
      ...
      if (this.classes.length === 0) {
        //! Проверяем Rest на пустоту
        this.element = "menu__item"; //! если пусто добавляем по умолчанию
  ...
  //     new Menu(img, altimg, title, descr, price, ".menu .container").render();
  //   });
  // }); //! первый способ формирования классов

  //! второй способ без классов с версткой на лету
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener("submit", (e) => {
      ...
      margin: 0 auto;
      `;
      form.insertAdjacentElement("afterend", statusMessage); //! analog append
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  function showModalByScroll() {
    //! Сравнение прокрутки если проскролили вниз то срабатывает
    if (
      window.pageYOffset + document.documentElement.clientHeight >=
      document.documentElement.scrollHeight - 1
    ) {
      openModal(modalSelector, modalTimerId);
      //! Удаляем эвент после первого срабатывания
...
function slider({
  ...
  //! inline style если нет стилей в css
